...
let add_edge (cfg : cfg) (src : string) (dst : string) : cfg =
  ...
  let update_src =
    { src_block with succs = src_block.succs @ [dst]} in
    let update_dst =
      { dst_block with preds = dst_block.preds @ [src]} in

      let new_blocks = cfg.blocks
      |> StringMap.add src update_src
      |> StringMap.add dst update_dst
    in
