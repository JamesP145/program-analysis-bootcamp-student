...
let build_env (_pairs : (string * int) list) : int StringMap.t =
  ...
  List.fold_left (fun _env (_name,_value) -> StringMap.add _name _value _env) StringMap.empty _pairs
...
let assigned_vars (_assignments : assignment list) : StringSet.t =
  ...
  List.fold_left (fun acc {var_name; _} -> 
    StringSet.add var_name acc
  ) StringSet.empty _assignments
